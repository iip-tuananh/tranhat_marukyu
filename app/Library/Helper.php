...
if(!function_exists('languageName')){
    function languageName($arrName){
        ...
        if (!is_array($arr)) {
            return $arrName; // Nếu không phải JSON, trả về nguyên văn
        }
        // $session =  Session::get('locale');
        // if($session != null){
        //     $code = Session::get('locale');
        ...
        foreach($arr as $item){
            if(isset($item->lang_code, $item->content) && $item->lang_code == $locale){
                return $item->content;
            }
        }
        // Nếu không tìm thấy, trả về ngôn ngữ mặc định (ví dụ: English)
        return $arr[0]->content ?? '';
    }
}
if(!function_exists('formatCurrency')){
    function formatCurrency($price_usd, $price_vnd){
        $currency = Session::get('currency', 'USD');
        $currency_rate = CurrencyRate::where('currency', $currency)->first();
        $currency_symbol = $currency_rate->nationalCurrency->symbol;
        if($currency != 'VND'){
            $price = $price_usd * $currency_rate->rate;
            // return $currency_symbol . ' ' . number_format($price, 0, ',', '.');

        }else{
            // return number_format($price_vnd, 0, ',', '.') . ' ' . $currency_symbol;
            $price = $price_vnd;
        }
        ...
        return $formatter->format($price) . $currency_symbol;
    }
}
