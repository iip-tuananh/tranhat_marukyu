						...
						<vs-table max-items="5" pagination :data="list">
							...
							<template slot-scope="{ data }">
								<vs-tr :key="indextr" v-for="(tr, indextr) in data">
									...
									<vs-td :data="tr.id">
										<router-link
											:to="{ name: 'editCateBlog', params: { id: tr.id } }"
										>
											<vs-button
												vs-type="gradient"
												size="lagre"
...
<script>
import ModalAdd from '../../layouts/modal/blog/addCategory.vue';
...
export default {
	...
	methods: {
		...
		listCategory() {
			this.loadings(true);
			this.listCateBlog({ keyword: this.keyword }).then(response => {
				this.loadings(false);
		...
		destroy() {
			this.loadings(true);
			this.destroyCateBLog({ id: this.id_item }).then(response => {
				this.listCategory();
				this.loadings(false);
