              ...
              <div class="row" v-for="(item, key) in objData" :key="key">
                <div class="col-md-3">
                  <div class="form-group">
                    <image-upload :oldImage="item.image" type="avatar" v-model="item.image" :title="'doi-tac'"></image-upload>
                  </div>
                </div>
                <div class="col-md-9">
                  <div class="form-group">
  ...
  methods: {
    ...
    savePartners(){
      ...
      this.savePartner({data:this.objData}).then(response => {
        this.loadings(false);
        this.$notify.success('Thêm đối tác thành công');
      }).catch(error => {
        this.loadings(false);
        this.$notify.error('Thêm đối tác thất bại');
      })
    },
    addObjPartner(){
      this.objData.push({
